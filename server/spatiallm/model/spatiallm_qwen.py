    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if (
            ...
            # 检查是否为预编码特征 (形状应该是 [batch, seq_len, hidden_size])
            # 原始点云数据的形状应该是 [batch, n_points, n_features] 其中n_features通常是6或7
            is_preencoded = (
                len(point_clouds.shape) == 3 and 
                point_clouds.shape[2] == self.config.hidden_size
            )
            ...
            if is_preencoded:
                ...
                for i in range(n_point_clouds):
                    point_features.append(point_clouds[i].unsqueeze(0))
            elif self.point_backbone is not None:
                # 使用点云编码器编码原始点云
                print("编码原始点云")
                ...
                raise RuntimeError(
                    "无法处理点云数据：没有点云编码器且输入不是预编码特征。\n"
                    "请确保在CUDA环境中编码点云，或提供预编码特征。"
                )

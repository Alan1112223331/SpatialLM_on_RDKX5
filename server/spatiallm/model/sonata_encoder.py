...
class Point(Dict):
    ...
    def sparsify(self, pad=96):
        ...
        if not _has_spconv:
            raise RuntimeError(
                "sparsify方法需要spconv库，但当前环境中不可用。"
                "这通常意味着您在CPU推理环境中尝试使用点云编码器。"
            )
...
class GridPooling(PointModule):
    ...
    def forward(self, point: Point):
        if not _has_torch_scatter:
            raise RuntimeError(
                "GridPooling需要torch_scatter库，但当前环境中不可用。"
                "这通常意味着您在CPU推理环境中尝试使用点云编码器。"
            )
...
class Sonata(PointModule, PyTorchModelHubMixin):
    def __init__(
        ...
        # 检查CUDA依赖是否可用
        if not _has_spconv:
            # 在CPU推理环境中，Sonata编码器不应该被实际使用
            # 这里我们允许实例化，但会在forward时抛出错误
    ...
    def forward(self, data_dict):
        if not self._cuda_available:
            raise RuntimeError(
                "Sonata点云编码器需要CUDA依赖(spconv)，但当前环境中不可用。\n"
                "这通常意味着您在CPU推理环境中尝试使用点云编码器。\n"
                "请确保：\n"
                "1. 在CUDA环境中进行点云编码，生成特征文件\n"
                "2. 在CPU环境中只进行推理，使用预编码的特征文件\n"
                "3. 如果您确实需要在此环境中进行点云编码，请安装CUDA版本的PyTorch和spconv"
            )

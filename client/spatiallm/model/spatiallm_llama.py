...
class SpatialLMLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward_point_cloud(self, point_cloud, device, dtype):
        ...
        if self.point_backbone_type == PointBackboneType.SCENESCRIPT:
            # Ensure coords are torch.int for TorchSparse v1.4 compatibility
            ...
            result = self.point_proj(encoded_features["context"].to(dtype))
            
            # Handle dimension mismatch: if result is 3D, we need to flatten appropriately
            if result.dim() == 3:
                # result shape: [num_spatial_points, tokens_per_point, hidden_size]
                # We need: [total_tokens, hidden_size] where total_tokens = num_spatial_points * tokens_per_point
                result = result.view(-1, result.size(-1))
                # Add batch dimension back: [1, total_tokens, hidden_size]
                result = result.unsqueeze(0)
            elif result.dim() == 2:
                # Add batch dimension: [1, num_tokens, hidden_size]  
                result = result.unsqueeze(0)

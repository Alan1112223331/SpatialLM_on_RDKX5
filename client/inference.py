...
def generate_layout(
    ...
    # Use longer timeout for CPU inference and fewer tokens for faster generation
    timeout = 300000.0 if "cpu" in str(model.device).lower() else 20.0
    if "cpu" in str(model.device).lower() and max_new_tokens > 1024:
        max_new_tokens = 512  # Reduce tokens for CPU inference
        print(f"Note: Reduced max_new_tokens to {max_new_tokens} for faster CPU inference")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=4096,
        help="Maximum number of new tokens to generate (reduced automatically for CPU)",
    )
